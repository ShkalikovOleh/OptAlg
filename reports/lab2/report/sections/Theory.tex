...
\begin{figure}[h!]
    \begin{center}
        \begin{tikzpicture}[node distance=1.5cm]
            ...
            \node[decision] (criterion) at (-5.5,-2) {Виконується критерій зупинки?};

            \node[process] (final_sort) at(-5.5,-4.7) {Sort};
            \node[startstop] (stop) at(-5.5,-6.5) {$x_{min} = x_1$};

            \node[process] (sort) at (-2.75, -2) {Sort};
            \node[process] (reflection) at (-0.5, -2) {Reflection};
            \node[decision] (ref_check) at (2.5, -2) {$f(x_1)\leq f(x_r)$ $f(x_r)<f(x_n)$};
            \node[decision] (exp_contr_check) at (6, -2) {$f(x_r)<f(x_1)$};

            \node[process] (contraction) at (6, 0.5) {Contraction};
            \node[process] (expansion) at (6, -4.7) {Expansion};

            \node[decision] (contr_check) at (2.5, 0.5) {$f(x_{contr}) < f(x_{n+1})$};
            \node[decision] (exp_check) at (2.5, -4.7) {$f(x_e) < f(x_r)$};

            \node[process] (shrink) at (-0.5, 0.5) {Shrink};
            ...
            --node[anchor=south]{$x_{n+1}=x_r$} ++(-7,0)--(criterion);

            \draw[arrow] (ref_check)--node[anchor=south]{ні} (exp_contr_check);

            \draw[arrow] (exp_contr_check)--node[anchor=east]{так} (expansion);
            \draw[arrow] (exp_contr_check)--node[anchor=east]{ні} (contraction);

            \draw[arrow] (expansion)--(exp_check);
            \draw[arrow] (contraction)--(contr_check);

            \draw[arrow] (contr_check)--node[anchor=south]{ні} (shrink);
            \draw[arrow] (contr_check)--node[anchor=west]{так}
            ++(0, -1)--node[anchor=south]{$x_{n+1} = x_{contr}$}++(-8, 0)--(criterion);
            \draw[arrow] (exp_check)--node[anchor=south]{так \;\; $x_{n+1} = x_e$}
            ++(-7, 0)--(criterion);
            \draw[arrow] (exp_check)--node[anchor=west]{ні}
            ++(0,1.2)--++(-7,0)--(criterion);


            \draw[arrow] (shrink)--++(-5, 0)--(criterion);
...
\begin{figure}[h!]
    \begin{center}
        \begin{tikzpicture}[node distance=1.9cm]
            ...
            \draw [arrow] (step6.west) -- ++(-3,0) -- ++(0,11.6) -- (step2);
        \end{tikzpicture}
    \end{center}
